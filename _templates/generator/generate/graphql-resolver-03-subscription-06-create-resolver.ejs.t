...
export const <%= h.changeCase.camel(locals.gqlResolver?.name) %>Resolver: GQL_SubscriptionResolvers['<%= h.changeCase.camel(locals.gqlResolver?.name) %>'] = {
  subscribe: withFilter(
    ...
    (payload: GQL_<%= h.changeCase.pascal(locals.gqlResolver?.common?.gqlReturnTypeName) %>, args: GQL_Subscription<%= h.changeCase.pascal(locals.gqlResolver?.name) %>Args, ctx: IGraphQLContext) => {
